...
export async function approveReconciliation(state, gateway) {
  const paymentAsset = ASSETS[1];
  await gateway.addTags(paymentAsset.urn, state.writeBack.tags);
  await gateway.updateDescription(
    paymentAsset.urn,
    "MandateGuard MG-204: 37 mismatched payment records quarantined and reconciled. Downstream settlement and executive revenue assets protected."
  );
